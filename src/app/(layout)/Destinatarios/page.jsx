                    ...
                    <tbody>
                        {userDB && userDB !== undefined && userDB.destinatarios && userDB.destinatarios !== undefined && Object.values(userDB.destinatarios).map((i, index) => {
                            return i.destinatario.toLowerCase().includes(filter.toLowerCase()) && <tr className={`text-[14px] border-b hover:bg-gray-100  ${index % 2 === 0 ? 'bg-gray-100' : 'bg-gray-100'} `} key={index}>
                                ...
                                <td className="w-32 p-3">
                                {i['pais']}{countries?.[i?.cca3]?.habilitado !== undefined && countries[i.cca3].habilitado === true ? <span className='text-green-400 text-[10px] py-2 px-3 rounded-[5px] bg-gray-800 inline-block'> habilitado: {countries?.[i.cca3]?.code} {countries?.[i.cca3].divisasPaisDestinatario}</span> :<span className='text-red-500 text-[10px]  py-2 px-3 rounded-[5px] bg-gray-800 inline-block'> inhabilitado</span>}
                                <br />
